...
<body>
    ...
    if (isset($_GET['delete_id'])) {
        ...
        $sqlDelete = "DELETE FROM gb_scheme_code WHERE scheme_code = '$deleteId'";
        if ($conn->query($sqlDelete) === TRUE) {
            echo "<script>alert('Deleted successfully.');</script>";
            header("Location: schcode.php");
        } else {
    ...
    if (isset($_POST['update_year'])) {
        ...
        $sqlUpdate = "UPDATE gb_scheme_code SET scheme_code = '$updateYear' WHERE scheme_code = '$currentYear'";
    ...
    if (isset($_POST['year'])) {
        $year = $_POST['year'];
        $sqlInsert = "INSERT INTO gb_scheme_code(scheme_code) VALUES ('$year')";
    ...
    <div class="content"> <!-- Apply the content class to the form's container -->
        <h2>Insert New scheme code</h2>
        <form method="post" action="">
            <label for="year">Year:</label>
            <input type="text" name="year" required>
            <input type="submit" value="Insert scheme code">
        </form>
    </div>
    <div class="clear"></div> <!-- Clear the float after the form -->
    <?php
    // Retrieve data from the gb_fy table
    $sqlSelect = "SELECT * FROM gb_scheme_code order by scheme_code";
    $result = $conn->query($sqlSelect);
    echo "<table>";
    echo "<tr><th>Year</th><th>Action</th><th>Action</th></tr>";

    while ($row = $result->fetch_assoc()) {
        $currentYear = $row["scheme_code"];
        echo "<tr>";
        echo "<td>$currentYear</td>";
        echo "<td><a href='?delete_id=" . $row["scheme_code"] . "'>Delete</a></td>";
        ...
        echo "<label for='update_year'>New Scheme Code:</label>";
        echo "<input type='text' name='update_year' required>";
        echo "<input type='submit' value='Update scheme code:'>";
        echo "</form>";
        echo "</td>";
        echo "</tr>";
    }
