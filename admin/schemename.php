...
<html>
<head>
    <title>List of Scheme Name</title>
...
<body>
    ...
    if (isset($_GET['delete_id'])) {
        ...
        $sqlDelete = "DELETE FROM gb_scheme_name WHERE scheme_name = '$deleteId'";
        if ($conn->query($sqlDelete) === TRUE) {
            echo "<script>alert('Deleted successfully.');</script>";
            header("Location: schemename.php");
        } else {
    ...
    if (isset($_POST['update_year'])) {
        ...
        $sqlUpdate = "UPDATE gb_scheme_name SET scheme_name = '$updateYear' WHERE scheme_name = '$currentYear'";
    ...
    if (isset($_POST['year'])) {
        $year = $_POST['year'];
        $sqlInsert = "INSERT INTO gb_scheme_name(scheme_name) VALUES ('$year')";
    ...
    <div class="content"> <!-- Apply the content class to the form's container -->
        <h2>Insert New Scheme Name</h2>
        <form method="post" action="">
            <label for="year">Year:</label>
            <input type="text" name="year" required>
            <input type="submit" value="Insert Scheme Name">
        </form>
    </div>
    <?php

    // Retrieve data from the gb_fy table
    $sqlSelect = "SELECT * FROM gb_scheme_name order by scheme_name";
    $result = $conn->query($sqlSelect);
    echo "<table>";
    echo "<tr><th>Year</th><th>Action</th><th>Action</th></tr>";

    while ($row = $result->fetch_assoc()) {
        $currentYear = $row["scheme_name"];
        echo "<tr>";
        echo "<td>$currentYear</td>";
        echo "<td><a href='?delete_id=" . $row["scheme_name"] . "'>Delete</a></td>";
        ...
        echo "<label for='update_year'>New Scheme Name:</label>";
        echo "<input type='text' name='update_year' required>";
        echo "<input type='submit' value='Update Scheme Name:'>";
        echo "</form>";
        echo "</td>";
        echo "</tr>";
    }
